...
//define tracks
char* tracks[] = {"track005.mp3","track006.mp3","track001.mp3","track002.mp3","track003.mp3","track004.mp3"};
//char** tracks = new char*[1000];
//int index = 0;

int trackNumber = 0;
int maxNumberOfTracks = 6;
...
void loop() {         
  ...
  if((!musicPlayer.playingMusic) && !stopped && !paused && (trackNumber < (maxNumberOfTracks-1))){
  ...
  if (musicPlayer.stopped()) {
    ...
    if (nextPressed) {
      ...
      if(trackNumber < maxNumberOfTracks-1){
        trackNumber = trackNumber +1;
        playTrack(trackNumber);
      }else{
        Serial.println("Not possible to go forward. TrackNumber is already the last one.");  
      }
    }
    ...
    if (prevPressed) {
      ...
      if(trackNumber > 0){
        ...
        Serial.println("Not possible to go backward. TrackNumber is already the first.");  
      }
